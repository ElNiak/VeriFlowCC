...
# TODO:
# - Should type, lint and format the code  -> Do not allow stopping if there are any issues.
# - Should suggest next steps or agents to run.
# - Should not run on every tool call, only on Write/Edit/MultiEdit.
# - Should decide if we need to compact context or not and if so, how.
