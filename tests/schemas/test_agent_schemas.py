...
class TestBaseAgentSchemas:
    ...
    def test_agent_input_valid_data(self) -> None:
    ...
    def test_agent_input_missing_required_fields(self) -> None:
        """Test AgentInput validation with missing required fields."""
        with pytest.raises(ValidationError) as exc_info:
            AgentInput(stage=VModelStage.CODING, context={})  # type: ignore[call-arg]

        assert "story_id" in str(exc_info.value)

    def test_agent_input_invalid_stage(self) -> None:
        """Test AgentInput with invalid stage value."""
        with pytest.raises(ValidationError):
            AgentInput(story_id="US-003", stage="INVALID_STAGE", context={})  # type: ignore[arg-type]

    def test_agent_output_valid_data(self) -> None:
    ...
    def test_agent_output_minimal_data(self) -> None:
        """Test AgentOutput with minimal required data."""
        data: dict[str, Any] = {"status": "error", "artifacts": {}}
    ...
    def test_agent_output_invalid_status(self) -> None:
        """Test AgentOutput with invalid status value."""
        with pytest.raises(ValidationError):
            AgentOutput(status="invalid_status", artifacts={})  # type: ignore[arg-type]

    def test_agent_output_with_errors(self) -> None:
...
class TestDesignSchemas:
    ...
    def test_design_input_valid_data(self) -> None:
    ...
    def test_design_input_missing_requirements(self) -> None:
        ...
        with pytest.raises(ValidationError) as exc_info:
            DesignInput(story_id="US-001", stage=VModelStage.DESIGN, context={})  # type: ignore[call-arg]

        assert "requirements_artifacts" in str(exc_info.value)

    def test_design_output_valid_data(self) -> None:
    ...
    def test_design_output_missing_specifications(self) -> None:
        """Test DesignOutput validation with missing design specifications."""
        with pytest.raises(ValidationError) as exc_info:
            DesignOutput(status="success", artifacts={})  # type: ignore[call-arg]
...
class TestImplementationSchemas:
    ...
    def test_implementation_input_valid_data(self) -> None:
    ...
    def test_implementation_output_valid_data(self) -> None:
...
class TestTestingSchemas:
    ...
    def test_testing_input_valid_data(self) -> None:
    ...
    def test_testing_output_valid_data(self) -> None:
...
class TestIntegrationSchemas:
    ...
    def test_integration_input_valid_data(self) -> None:
    ...
    def test_integration_output_valid_data(self) -> None:
...
class TestSchemaValidationRules:
    ...
    def test_status_must_be_valid_enum(self) -> None:
        ...
        for status in valid_statuses:
            output = AgentOutput(status=status, artifacts={})  # type: ignore[arg-type]
            assert output.status == status

    def test_metrics_must_be_dict(self) -> None:
        """Test that metrics field must be a dictionary."""
        with pytest.raises(ValidationError):
            AgentOutput(status="success", artifacts={}, metrics="invalid")  # type: ignore[arg-type]

    def test_errors_must_be_list_of_strings(self) -> None:
        """Test that errors field must be list of strings."""
        with pytest.raises(ValidationError):
            AgentOutput(status="error", artifacts={}, errors=["valid", 123, "invalid_number"])  # type: ignore[list-item]

    def test_next_stage_ready_is_boolean(self) -> None:
        ...
        with pytest.raises(ValidationError):
            AgentOutput(status="success", artifacts={}, next_stage_ready="not_boolean")  # type: ignore[arg-type]
